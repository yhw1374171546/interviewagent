...
| **JD 语义缓存** | 相似 JD 复用解析结果（embedding 余弦相似度 ≥0.9 命中），省重复 LLM 调用；Web 生产已启用 |
| **能力落地（Web 全启用）** | 多评委仲裁 + 评分校准 + 自适应难度 + 语义缓存全部接入生产面试链路，回归测试锁住不丢失 |
| **评分校准（Web 已启用）** | 按「关键词命中率 vs LLM 评分」纠正高低估：真实评测 **MAE 1.2→0.76（-37%）、Pearson 0.773→0.952**；生产面试链路已接入 |
...
| **评估可解释性（CoT）** | 评估 prompt 结构化思维链（先逐步分析再给结论）+ DeepSeek reasoning_content 落库，前端可折叠查看「推理过程」——评分不再是黑盒 |
