...
| **RAG 面经** | 内置 22 条面经 + [docs/knowledge](docs/knowledge/README.md) 知识库（397 条 Agent 面经），轻量检索器（关键词/相似度，零向量库依赖），面试报告参考答案「检索增强生成」——优先给真实面经答案而非关键词要点 |
...
| 参考答案平均长度 | 36 字（关键词罗列） | **100 字**（RAG 面经） |
| 参考答案信息密度 | 7.2 | **14.3** |
| 面经库覆盖（22 内置 + 397 知识库 = 419 条） | — | **43%**（严格阈值，无噪音匹配） |
