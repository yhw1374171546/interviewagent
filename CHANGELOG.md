...
### 2026-08-15 20:50 | 评估体验优化：等待缩短（评估∥追问并行）+ 流式思考过程（SSE）

**背景**: 用户反馈每次回答后等待太久（评估 LLM + 追问 LLM **串行**，总等待 = 两者之和），且等待期间只有转圈——希望「先把思考过程给用户看，最后再弹评分」。

**做了什么**:
1. **等待缩短 — 评估与追问决策并行** — `evaluator.keyword_analysis()` 新增确定性关键词层（0 LLM 秒级，含边界预判）；`submit_answer`/`submit_answer_stream` 改为 `asyncio.gather(evaluate, decide)` 并行执行——decide 不依赖 LLM 评语（用关键词层构造轻量评估上下文），总等待 ≈ **max(评估, 追问)** 而非 sum（约省 30-50%）；边界输入/成本降级时保持串行（decide 由评估器兜底）
2. **流式思考过程 — `POST /api/interviews/{id}/answer/stream`（SSE）** — 事件序列: `analyzing`（关键词命中率/已命中/未命中，秒出第一屏）→ `analysis`（LLM 结构化思维链 CoT，打字机逐字展示）→ `evaluation`（完整评分 + 追问/下一题）→ `done`；前端 `sendAnswer`/`submitCode` 改走 SSE，失败自动回退普通 POST
3. 追问决策逻辑抽 `_resolve_follow_up`（submit_answer 与 stream 版共用，防两处漂移）

**为什么这么做**: 「等待体验」是 LLM 产品的核心 UX——用户不关心你调了几次 LLM，关心的是「等待时能看到什么」。关键词层是客观事实（0 成本）先展示，CoT 是上一轮 P0 的产出正好拿来当「思考过程」展示——两轮工作自然衔接，评估从「黑盒转圈」变成「先客观分析 → 再推理过程 → 最后评分」。

**实测**: 457 测试全绿（+7：keyword_analysis 边界/正常、并行后 submit_answer 行为不变、SSE 事件序列、指标记录）；ruff 零错误；`node --check` 通过；server 路由 21 条（新端点注册）。

**经验**: ① 「等待缩短」第一选择是**并行化串行 LLM 调用**——评估与追问的数据依赖只有关键词层（确定性），拆出来就能并行；② 流式的体验价值不在「快」，在「有进度」——关键词分析秒出 + 思考过程逐字滚动，用户感知的等待时间远小于真实时间（感知心理学）；③ 新旧端点并存 + 前端失败回退 = 流式改造零风险——SSE 挂了自动走普通 POST，不会卡死。
